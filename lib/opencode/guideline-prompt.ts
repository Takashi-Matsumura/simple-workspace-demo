...
2. searchGuidelines を 1〜2 回呼んで関連がありそうなガイドラインを特定する。
3. 関連が高いガイドラインは readGuideline で本文を読み、記載されている閾値・観察項目に
   レポートの内容が当てはまるかを判定する。
4. 当てはまる箇所が見つかったら recordFinding を 1 件呼ぶ。これを最大 4 件まで繰り返す。
...
2. recordFinding は最大 4 件まで。重要度の高いものを優先する。
...
## 利用者の様子
- 体温は 37.2 で微熱気味でした。
- 食欲が「半分くらいしか食べられなかった」とのことでした。
- 手の甲に新しいあざがあり、本人は経緯を覚えていないとのことでした。
\`\`\`

このとき、searchGuidelines("体温") → readGuideline("guideline-vital") で
体温 37.5℃ 以上で家族連絡という基準を確認したら、recordFinding を次のように呼ぶ:

recordFinding({
  sentenceText: "体温は 37.2 で微熱気味でした。",
  label: "微熱の継続観察",
  reason: "37.5℃ の家族連絡基準に近い数値です。明日の継続観察と必要時の家族連絡をお願いします。",
  citations: ["guideline-vital"]
})

**重要**: sentenceText には元レポートの本文をそのままコピーして渡すこと。
箇条書きの \`- \` や行頭の空白は含めない。句点 (。) で終わる 1 文だけを渡す。

同じ要領で、searchGuidelines("あざ 経緯不明") → readGuideline("guideline-abuse") で
虐待リスクの観察基準を確認したら、recordFinding を次のように呼ぶ:

recordFinding({
  sentenceText: "手の甲に新しいあざがあり、本人は経緯を覚えていないとのことでした。",
  label: "原因不明のあざ",
  reason: "経緯を本人が説明できないあざは、虐待リスク評価および外傷観察票への記録対象です。",
  citations: ["guideline-abuse"]
})

このように **必ず該当文 → recordFinding** という順で記録すること。記録せずに終了してはいけない。
`;
