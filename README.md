...
[http://localhost:3000](http://localhost:3000) を開くと `/login` にリダイレクトされます。**新規登録は既定で閉鎖** されているので、最初の 1 ユーザだけ `.env` に `ALLOW_REGISTRATION="true"` を設定してから `/register` で作成し、戻して再起動してください ([セキュリティ考慮事項](#セキュリティ考慮事項) 参照)。ログイン後に Workspace を作って OpenCode パネル / 訪問レポートパネルを開けます。
...
| `COOKIE_SECURE` | Cookie の Secure 属性。未指定/`auto` で production 時のみ自動付与。HTTP の LAN 配信では `false` |
| `ALLOW_REGISTRATION` | 新規登録 API を許可するか。`true` 以外は `/api/auth/register` が 403。既定 = 閉鎖 |
...
| `SANDBOX_CPUS` | Sandbox コンテナの CPU 数 (小数可、既定 `1.0`) |
| `SANDBOX_MEMORY` | Sandbox コンテナのメモリ上限。サフィックス `k`/`m`/`g` 可 (既定 `1g`) |
| `SANDBOX_PIDS` | Sandbox コンテナの PidsLimit (既定 `256`) |
...
- **Sandbox 機能はホスト Docker daemon の socket を共有することで実現している**ため、アプリコンテナが RCE された場合の影響範囲はローカル開発時と変わらず大きい。LAN 公開や公開ホストでの運用は引き続き非推奨です ([セキュリティ考慮事項](#セキュリティ考慮事項) 参照)
...
本デモは家庭内 LAN 用途を想定したコード側のハードニングを入れていますが、**ホスト Docker socket を共有している以上、アプリコンテナ内 RCE はホスト乗っ取りに直結する** という構造的リスクは残ります。LAN 公開する場合の最低限のチェックリスト:

- [ ] `SESSION_SECRET` を 32 文字以上のランダム値に差し替えた
- [ ] `ALLOW_REGISTRATION` を空 (= 既定: 閉鎖) にした。最初の 1 ユーザを作るときだけ `"true"` で起動 → 登録 → 戻して再起動の運用
- [ ] `SANDBOX_CPUS` / `SANDBOX_MEMORY` / `SANDBOX_PIDS` をホストの余力に合わせて調整 (未指定なら CPU 1.0 / Mem 1GiB / PIDs 256)
- [ ] `/api/auth/login` と `/api/auth/register` は IP+username で **60 秒に 5 回失敗 → 15 分ロック** されることを把握 (Retry-After ヘッダ付きで `429`)
- [ ] Sandbox の既定ネットワークは `--network none` に変更済。**npm i などでネットが必要な操作は UI トグルを OFF (= bridge)** にしてから実施

挙動上の注意:
- レート制限は **単一プロセスの in-memory** 実装。PM2 cluster や水平スケールでは無効化される
- 既存ユーザのコンテナ (旧仕様: bridge / リソース無制限) はログインだけでは新仕様に置き換わらない。**Sandbox トグルを 1 度 OFF→ON すると recreate されて新仕様に揃う** (Volume は維持)
- `server.ts` には reverse proxy が無いので `x-forwarded-for` 不在時はレート制限キーの IP 部分が `unknown` に縮退する

別 Issue で扱う構造的リスク (本リポジトリのコードでは塞がない):
- HTTPS 終端 (Tailscale / Caddy / Let's Encrypt のリバプロ前段)
- Docker socket-proxy 経由で読取 API のみに制限
- VPN (Tailscale / WireGuard) 内のみ公開
...
| **ネットワーク** | Sandbox **ON** (既定) = **`--network none` で外部完全遮断** (loopback のみ) / Sandbox **OFF** = `bridge` (通常通り外部接続可、npm i 等で必要な時だけ) |
| **リソース上限** | env 未指定時の既定 = CPU `1.0` / Memory `1 GiB` / PIDs `256`。`SANDBOX_CPUS` / `SANDBOX_MEMORY` / `SANDBOX_PIDS` で上書き可 |
| 起動契機 | **ログイン / 新規登録の API 完了直後に fire-and-forget で `ensureContainer`**。Docker daemon 不在でもログイン自体は通る (失敗はサーバログのみ) |
| Sandbox トグル | workspaces パネルヘッダの **Sandbox ON/OFF** (サーバ既定 = ON / 隔離、localStorage に永続)。トグル切替で `POST /api/shell { action: "restart", networkMode }` を呼び、コンテナを force remove → 指定モードで再生成 |
...
1. (初回のみ) `.env` で `ALLOW_REGISTRATION="true"` を一時設定 → 開発サーバ再起動 → `/register` で新規ユーザを作成 → 戻して再起動
2. ログイン直後に `docker ps --filter label=app=simple-workspace-demo` で `simple-workspace-shell-<uid>` が `Up` になっていれば OK
3. フッターの **Shell** ボタンで端末パネルを開き、`whoami` が `root`、`cat /etc/os-release` が Ubuntu 24.04 であることを確認
4. Sandbox **ON** (既定) のまま `ping -c 1 8.8.8.8` → タイムアウトすればネット遮断成功 (`--network none`)
5. workspaces パネルヘッダの **Sandbox** トグルを **OFF** → Shell パネルは自動再接続。`ping -c 1 8.8.8.8` の応答が返る (bridge 接続)
6. `docker inspect simple-workspace-shell-<uid> --format '{{.HostConfig.NanoCpus}} / {{.HostConfig.Memory}} / {{.HostConfig.PidsLimit}}'` で `1000000000 / 1073741824 / 256` (既定値) が見えれば OK
7. `/root/foo` などにファイルを作って `docker rm -f simple-workspace-shell-<uid>` → Shell パネルの再起動ボタンで再生成しても `/root/foo` が残っていれば Volume 永続 OK
8. ログアウト → `docker ps -a --filter label=app=simple-workspace-demo` でコンテナが `Exited` になる (停止のみ。Volume は残る)
...
| `POST` | `/api/shell` `{ "action": "start" }` | 既存コンテナを start (無ければ既定 `none` で作成) |
