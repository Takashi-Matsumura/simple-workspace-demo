...
ベースは [`myworkspaces`](https://github.com/Takashi-Matsumura/myworkspaces.git) からログイン / Workspace / ホワイトボード / パネル拡張機構を移植し、PostgreSQL や opencode 連携など多数のパネル群は削ぎ落とした構成です。さらに myworkspaces のサンドボックス機能 (ログインユーザごとの Docker コンテナ + xterm 端末) を最小構成で追加しています。
...
├── docker.ts                               # dockerode ラッパー (ensureContainer / execShell / removeContainer)
...
myworkspaces 由来のサンドボックス機能を最小構成で移植したパネルです。フッターの **Shell** ボタンでパネルが開き、xterm.js + WebSocket でユーザ専用 Docker コンテナに対話できます。
...
| Volume | `simple-workspace-data-<sanitizedUserId>` を `/root` にマウント (再起動しても永続) |
| **ネットワーク** | **`--network none` で外部ネット完全遮断**。`ping`/`curl`/DNS いずれも届かない (loopback のみ可) |
| 起動契機 | 初回 WS 接続時に lazy 確保 (login API には触らない) |
| 機能トグル | workspaces パネルヘッダの **Sandbox ON/OFF** (default OFF, localStorage 永続)。OFF 時は Shell ボタン非表示 + 開いている Shell パネルを自動 close |
| ライフサイクル | ログアウトでは何もしない。手動で `docker rm -f` してもパネル再オープンで自動再生成 |
| Shell パネルの再起動ボタン | コンテナを `force remove` → 再作成 → 再接続 (Volume は維持) |
| WebSocket エンドポイント | `/api/shell/ws` (`server.ts` がカスタムサーバとして upgrade を捌く) |
| 認証 | 既存の `mw_session` 署名 Cookie を WS upgrade 直前に検証。Origin が host と不一致なら 403 |
...
| メソッド | パス | 用途 |
| --- | --- | --- |
| `GET` | `/api/shell` | コンテナ状態 (`running` / `stopped` / `absent`) とイメージ準備状況を返す |
| `POST` | `/api/shell` `{ "action": "start" \| "restart" }` | コンテナを確保 / 強制再生成 (Volume は保持) |
...
- アイドル停止は未実装。コンテナは作成後 `sleep infinity` で常駐し続ける
- 不要になったリソースは `docker ps -a --filter label=app=simple-workspace-demo` / `docker volume ls --filter label=app=simple-workspace-demo` で確認し、`docker rm -f` / `docker volume rm` で掃除してください
