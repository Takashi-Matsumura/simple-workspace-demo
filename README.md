...
OpenCode パネルにはモードセレクタ (`RAG` / `Agentic` / `Coding`) があり、同じ質問を別アプローチで投げて挙動の違いを比較できます。
...
- **Coding** (`POST /api/opencode/coding`): 上記 2 つに加えて `writeFile` / `readFile` / `listFiles` / `deleteFile` を持つ。LLM が社内文書を調べた結果を Markdown ファイルとして仮想 FS に保存できる。最大 10 ステップ。

検索対象の社内文書 9 件は `lib/opencode/corpus.ts` にハードコードしてあります（仕様 3 件 / FAQ 3 件 / 障害履歴 3 件）。

### Coding Agent の仮想 FS

Coding モードのファイル操作はホスト FS には触れず、SQLite 上の `WorkspaceFile` テーブル（`workspaceId + path` で UNIQUE）に閉じています。

- パスは `[a-zA-Z0-9._\-/]` のみ、`..` 禁止、最大 200 文字
- 1 ファイルあたり最大 64KB、Workspace あたり最大 100 ファイル
- パネル右側のサイドバー (`/api/opencode/files` を叩く) に一覧と本文プレビューが出る
